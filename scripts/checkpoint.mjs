...
const scoped = {
  ...
  'TASK-090': ['按用户独立主数据决定建立 KFF 客户/会话/商品/订单 ID 与品牌合同；复合外键和只读历史保证唯一归属，其他站点不接入；实际支付与交付合同待后续', ['docs/decisions/017-independent-business-data.md','packages/contracts/src/order.ts','packages/core/src/orders.ts','supabase/migrations/20260912003348_g3_owned_order_snapshots.sql','tests/integration/orders.test.ts'], ['contracts','integration','web','production']],
  'TASK-093': ['显式币种/精度依据与整数金额、不可变商品版本、启用/停用、组合报价预览和确认；旧报价/客户版本/期限复核，订单固定价格、交付和条款、幂等及取消；订单不冒充支付核实，公共结账/实际支付另验', ['packages/contracts/src/order.ts','packages/core/src/orders.ts','apps/web/components/order-workbench.tsx','supabase/migrations/20260912003348_g3_owned_order_snapshots.sql','tests/contracts/order.test.ts','tests/integration/orders.test.ts','tests/browser/web/orders.spec.ts'], ['contracts','integration','web','production']],
  'TASK-094': ['按用户指定 Stripe 实现单次托管 Checkout、固定金额与请求、商户/模式绑定、持久签名收件、事件/支付对象去重、Session 与 PaymentIntent 服务器核验、故障恢复与测试收入隔离；仅本地合成和真实 SDK 请求合同验证', ['docs/decisions/018-stripe-phase-one.md','packages/contracts/src/payment.ts','packages/core/src/stripe-gateway.ts','packages/core/src/payments.ts','packages/core/src/payment-worker.ts','apps/worker/src/main.ts','apps/web/app/api/[[...path]]/route.ts','apps/web/components/stripe-payment.tsx','apps/web/app/payment-return/page.tsx','supabase/migrations/20260912012250_g3_stripe_payments.sql','tests/contracts/payment.test.ts','tests/integration/payments.test.ts','tests/browser/web/payments.spec.ts','scripts/production-smoke.ts'], ['contracts','integration','web','production']],
  'TASK-095': ['部分与多次退款、持久额度预占、签名关联事件、原支付与完整退款/争议查询、原请求幂等恢复、带 token 租约、不可变资金变动与人工调整/冲销、订单退款页面；本地合成、真实 SDK HTTP 合同和进程中断验证，实际 Stripe 联调按用户要求后置', ['docs/decisions/019-deferred-stripe-configuration.md','packages/contracts/src/refund.ts','packages/core/src/stripe-financial-contract.ts','packages/core/src/stripe-financial-gateway.ts','packages/core/src/refunds.ts','packages/core/src/refund-reconciliation.ts','packages/core/src/refund-worker.ts','apps/web/components/stripe-refunds.tsx','supabase/migrations/20260912022322_g3_stripe_refunds.sql','tests/contracts/refund.test.ts','tests/integration/refunds.test.ts','tests/helpers/refund-process.ts','tests/browser/web/refunds.spec.ts','scripts/production-smoke.ts'], ['contracts','integration','web','production']],
};
for (const card of ledger.tasks) {
  ...
  if (['TASK-090','TASK-093'].includes(card.task_id)) {card.evidence_refs.push('docs/tasks/g3-owned-orders.md','docs/decisions/017-independent-business-data.md','docs/decisions/018-stripe-phase-one.md');card.known_blockers=['自有商品、报价与订单基础已验，Stripe 支付代码已按用户选择实现并本地验证；退款与调整代码已本地验证，实际 Stripe 测试联调、交付及访客自主结账尚待完成，未发生真实收款'];}
  if (card.task_id==='TASK-094') {card.evidence_refs.push('docs/decisions/018-stripe-phase-one.md','docs/tasks/g3-stripe-payments.md','docs/api/stripe-payments.md');card.known_blockers=['实际 Stripe 测试商户及 API/端点签名密钥尚未配置；真实托管收银台、Webhook 投递与服务器查询、密钥轮换和生产网络验收未执行，不计 TASK-094 或 G3 完整验收'];card.continue_within_authorized_scope=true;card.evidence_refs.push('docs/decisions/019-deferred-stripe-configuration.md');}
  if(card.task_id==='TASK-095'){card.evidence_refs.push('docs/tasks/g3-stripe-refunds.md','docs/api/stripe-refunds.md','docs/decisions/019-deferred-stripe-configuration.md');card.known_blockers=['本地代码范围已完成；实际 Stripe 商户与密钥配置、真实网络和端点联调按用户要求后置；TASK-096 交付及 ACC-29 完整验收尚未完成'];card.pause_after_scope_requested=true;}
}
ledger.execution_state={state:'PAUSED_BY_USER',after_task:'TASK-095',reason:'用户要求退款部分做完后暂停',shutdown_requested:false,full_delivery:false};
