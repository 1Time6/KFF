...
本页保留收款阶段的历史检查记录。后续用户已要求 Stripe 实际配置后置，并在退款与调整完成后暂停；当前进展与验证以 [退款阶段记录](g3-stripe-refunds.md)、[当前状态](../next-step.md) 和最新检查点为准。此前关机要求已改为保存并通知。
...
实际 Stripe 测试商户、API 密钥和 Webhook 签名密钥尚未配置；未进行真实 Stripe 网络调用、真实收银台测试、外部 Webhook 投递、密钥轮换或公网部署。配置已由用户明确后置，之后按 `docs/api/stripe-payments.md` 与 `config/stripe-secrets.example.json` 在本机私密位置配置。密钥不进入聊天、代码或证据。

当时只标记 TASK-094 的本地代码范围 `DONE_SCOPED`。退款后来在 TASK-095 实施；交付、人工回复/接管、其他媒体/AI、设备、持续运行及完整项目仍未完成，不计 E2-MIN、G3 或完整交付。最新指令是退款阶段完成后暂停并保存通知，不执行关机。
