...
依据 ADR-018 与 TASK-094。KFF 保留自有客户、会话、商品、订单和付款凭据，只把托管收银台与付款处理交给 Stripe。当前收款范围是运营人员为已确认订单生成单次付款链接；后续已实现 [退款与调整](stripe-refunds.md)，交付与访客自主下单尚待实施。

## 配置实际测试账号

用户已明确实际商户、API 密钥和 Webhook 签名密钥配置后置，以下留作之后联调的步骤。本地合成验证不会发送真实 Stripe 请求，也不能替代该步骤。密钥放在本机私密配置或部署环境的密钥管理器，不填入页面、聊天、提交文件或日志。
...
管理员暂停连接或组织/品牌停止后，新的外部创建被拒绝。已经提交的对象继续只读核对，合法回调仍接收。停止开关不会使已发出的 Stripe 付款链接失效，也无法撤回已到达 Stripe 的退款；当前没有远程过期收银台操作。退款权限和新增事件见 [退款与调整](stripe-refunds.md)，实际网络行为仍须后置联调。
...
`VERIFIED_TEST_PAID` 明确表示测试付款；只有 LIVE 且非合成的已核实凭据产生 `VERIFIED_PAID`。客户档案分别统计真实付款和测试记录，人工成交标签不影响该统计。这里是原始付款总额凭据；退款、争议和人工调整在独立财务记录中保存，不能直接当作 Stripe 扣除所有手续费后的结算净额。
