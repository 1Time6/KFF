# KFF 项目规划 v2.0：完整业务链复刻与 Codex 分步实施

> 修订日期：2026-09-11｜状态：待实施工程规划｜用途：产品总纲、架构约束、Codex 任务拆解和验收依据。
> 基础文件：用户上传《项目规划.md》v1.0。原文件保持不变，本文件为独立修订版。
> 已确认的是文档范围与设计。本文件不证明平台权限已取得、代码已实现、设备已联通或产品已验收。
...
G0 资料追溯、存量代码、真实入口和最小可行性试验
  → G1 单账号、单环境、单动作的可靠执行闭环
  → G2 采集、结果转任务、RPA 模板与小规模调度
  → G3 内容生产、接待、SCRM、网站成交与交付
  → G4 Android、iOS、云机与硬件盒子逐项接入
  → G5 容量、持续运行、恢复、升级与完整范围验收
```

G0 提前验证设备和平台路径；故障处理、权限与测试从 G1 开始。G3、G4 中无依赖的部分可在合同稳定后并行。`PLAN-F3-*` 中的浏览器执行器前置到 G1；这些编号保留为历史业务归属，不能再按编号机械地拖到后期。

### 0.3 Codex 首次只做什么

先读第 1、15、19、22、30 节及本次对应任务行。第一次执行只允许资料与仓库只读检查，输出最小下一任务并停止；不要一次性开发本文件全部内容。用户批准某个开发任务后，按其既定范围执行，无需对每次普通编辑重复确认。

总纲不直接复制为 `AGENTS.md`。项目规则应保持短小，并索引总纲和当前任务；一个大文件提供完整依据，不意味着每次运行都要把全文塞进指令上下文。[O01](#ref-o01)[O02](#ref-o02)

### 0.4 v2 修订定位

本版保留原有 21 个主体章节并修订冲突处，新增第 22 至 30 节作为执行细则。修订重点：
...
| 来源编号 | 材料或环境 | 本版依据与边界 |
| --- | --- | --- |
| S01 | 用户上传《项目规划.md》v1.0 | 已读取当前对话完整正文，并检查容器中的原始文件；这是本版的结构、术语和需求主来源 |
| S02 | 《【客番番】产品介绍(3).pdf》 | 使用当前对话已呈现的页面图像与文本核对厂商声明；产品说明不能替代实机结果 |
| S03 | 《产品优势及售后服务.doc》 | 使用当前对话已呈现的解析页核对 RPA、AI、客户管理、指纹和服务声明 |
| S04 | `2026-09-11 17-57-53.mp4` | 原文件存在于当前容器，本版没有重新完成全段视频或音频审阅；时间点继续标为历史转述 |
| S05 | 原执行 ZIP、110 项需求、53 项子规格、90 项验收等 | 当前可访问容器未见这些执行文件；没有重建或猜测其原编号，逐项映射仍待取得实际文件 |
| S06 | 用户 Windows 工作区与其他仓库 | 原版记录当时 KFF 目录只有 Git 元数据。本次没有访问该 Windows 电脑，不能把旧记录当成当前仓库状态 |
| O01 起 | 第 20 节官方技术文档 | 本次新增核验与原版保留入口分别标注；文档成立不代表用户账号已获准 |

S01 原始文件 SHA-256：

```text
7b02f3a2d01e8823acbf7408604acfec50ad89066784d4c9921727e052f525cc
```

原对话称历史执行包包括 110 项主需求、53 项子规格、20 个工作区、47 个工作包、90 项验收、25 段视频证据、13 个接口操作、12 份 JSON Schema、91 个文件。这些是历史交付声明；本版不复述为已验证资产。

本版沿用 S01 自行建立的 `MOD-*`、`UI-*`、`PLAN-*`、`ACC-*` 标识。取得 S05 后添加映射，保留其中额外范围。旧包缺失可阻塞“完整对标冻结”，但不必阻塞依据明确的 G0/G1 最小验证。

### 1.3 阅读规则

- **对话明确方向**：来自用户原始完整复刻目标，或原分析明确列出的业务范围。
- **原方案建议**：例如 Next.js、Supabase、独立 Worker、分期实施；尚不等于用户最终选型。
- **本次工程建议 / v2 修订**：第 0 节及第 22 至 30 节为新增设计；第 5、8、9、12、15、17、19 至 21 节按修订决定更新。未标注为已验证事实的字段、接口、工作包、质量目标和治理机制均为拟建要求。
...
每一步记录业务对象 ID 和事件 ID。来源不可得时保留未知。E2 测试询盘与支付测试模式可验证集成，实际客户成交另行记录。真实执行器通过 E1 仅证明选定范围，不代表整个项目已经验证一半。
...
S01 只记录了其编制时检查的 KFF 工作区，本版没有再次访问用户电脑。每次启动按当时授权的目录重新检查，不扫描无关个人目录。没有可靠代码证据的模块按待核实处理；没有经过验证的接口不写成现成依赖。
...
本节是 v2 的主要顺序修订。保留 31 个原 `PLAN-*` 主包，增加 4 个 `PLAN-X-*` 工程增强包。它们是功能责任单元；实际执行顺序由 G0 至 G5 和第 28 节细任务依赖决定。旧编号中的 F 数字不再作为开始时间。
...
| G1 技术闭环 E1 | 单账号、单环境、单动作、队列、Agent、真实结果与恢复 | 只读和写入分开验收；写入有许可与远端证据，关键故障通过 |
| G2 竞品核心 | 采集、精确目标转换、模板、计划、环境与小规模矩阵 | 每种动作独立可用；错误可诊断、修复可离线回归 |
| G3 成交闭环 E2 | 内容、统一接待、AI、SCRM、网站、订单和交付 | 经授权测试贯通内容到交付，对账、接管和真实范围明确 |
...
| PLAN-F1-08 首链验收 | G3 E2 成交链；不能提前冒充 G1 已完成 | E2 内容→发布→主动咨询→订单→核实付款→交付→来源报告；附真实范围与异常恢复记录 |
| PLAN-F2-01 采集与结果台 | G2，依赖 G1 账号与可靠执行 | 数据源适配、分页增量、checkpoint、Excel、去重和结果转任务 |
| PLAN-F2-02 线索与资格引擎 | G2 资格及人工证据；G3 模型分析增强 | 证据分析、人工改判、身份候选；复用首期发送资格检查，扩展来源用途与线索规则，验证高分不能突破资格 |
...
| PLAN-X-01 失败诊断包 | G1 D0；G2 D1/D2；G5 保留与删除审查 | D0/D1/D2 分级、诊断采集、脱敏、受控存储和共享包；不得把原始 trace 默认发送给模型 |
| PLAN-X-02 离线 Fixture 回归 | G1 本地合成页；G2 多版本回归 | 合成或脱敏样例、网络隔离、旧/新版本回归及受控真实复测；不宣称静态页面完整重演线上网站 |
| PLAN-X-03 环境能力专项 | G0 范围决定；G1 隔离；G2 供应商适配 | ENV-L1 会话隔离、ENV-L2 正式供应商能力与 ENV-L3 单独待决研发；保留依赖及未验证声明 |
...
| MOD-04 线索 | UI-05、15 | G2 资格与证据；G3 模型增强 | ACC-18、19、20、24 |
...
| MOD-12 运营服务 | UI-16、19、20 | G2 运行诊断；G3 经营；G5 交接 | ACC-36、49、52、53、57、64、65、67、70 |
...
G3 的 E2 支付与询盘链路可先使用明确标注的授权测试账号、测试询盘和支付测试模式；这证明技术集成，不代表真实营业收入或转化提升。生产渠道与真实业务成果另行核验，证据中必须区分测试环境、真实环境中的受控测试、实际客户成交。
...
| 编号 | 场景 | 必须观察到的结果 | 层级 |
| --- | --- | --- | --- |
| ACC-37 | 诊断包秘密与隐私扫描 | D0/D1 导出不含令牌、Cookie、密码、验证码和未必要客户正文；扫描失败阻断共享 | L1/L2 |
...
| ACC-46 | 独立环境隔离 | 受控测试中会话存储不串环境，账号绑定正确；普通隔离不记为全部指纹能力通过 | L2/L3 |
| ACC-47 | 供应商启动停止与接口超时 | 启动结果未知先查询，不多建实例；停止受理与进程终止分开，版本失效有记录 | L2/L3/L4 |
| ACC-48 | URL、文件与命令输入安全 | 阻断元数据/未授权内网、重定向逃逸、路径穿越、任意 shell 与非白名单命令 | L1/L2 |
...
| ACC-64 | 原范围与交付分母 | 原编号不丢失，未取得旧包不猜覆盖；BLOCKED 不默默移出完整交付分母 | 文档检查 |
...
| 编号 | 事项 | 当前状态与处理 | 责任岗位 |
| --- | --- | --- | --- |
| OPEN-01 | 原始视频、PDF、DOC 与执行包 | S01 已读，S02/S03 使用会话可见页；视频未重新全审，S05 尚未取得；保留证据等级和映射缺口 | 产品负责人 |
| OPEN-02 | 既有系统真实仓库 | KFF 空仓库为 S01 的历史记录；本次未访问用户电脑，启动时重新查明现状和存量 | 技术负责人 |
| OPEN-03 | 首发业务与首个平台 | G1 先验证单账号执行；E2 保留内容到主动咨询链；具体账号、动作、地区与写入许可待核实 | 业务负责人 |
...
后续开始开发时，先按第 28 节 G0 任务执行资料与代码盘点，再形成最小开发任务。当前交付仅为这份 Markdown 规划；没有创建应用、接入生产、发送消息或采购设备。首次可复制指令见第 30 节。

建议输出位置：

```text
KFF_项目规划_v2.0_Codex执行版.md # 本文：范围与总设计
...
这些附属文件均为下一阶段建议产物，本次未创建。实际代码目录在查明存量后确定；不存在适用存量时，候选目录为 `apps/web`、`apps/worker`、`apps/agent` 及 `packages/contracts`、`packages/core`、`packages/adapters`。目录边界只表示职责，不要求独立部署每一个模块。
...
本版材料主来源是 S01。S02/S03 只用于核对资料上列出的功能与声明；S04 的视频细节仍保持历史转述等级。资料自身的宣传不能作为稳定性结果。

| 编号 | 材料 | 定位与用途 |
| --- | --- | --- |
| <a id="ref-s01"></a>S01 | 用户上传《项目规划.md》v1.0 | 全文：本版保留其模块、页面、主包和验收；第 1、15、21 节含原版检查边界 |
| <a id="ref-s02"></a>S02 | 《【客番番】产品介绍(3).pdf》 | 第 3 至 5 页业务链与账号管理，第 8 至 12 页 RPA 和设备，第 15 至 16 页服务 |
| <a id="ref-s03"></a>S03 | 《产品优势及售后服务.doc》当前会话解析页 | 第 1 至 4 页指纹/采集/RPA/AI/SCRM，第 7 至 9 页人员与更新服务 |
| <a id="ref-s04"></a>S04 | `2026-09-11 17-57-53.mp4` | 本次未重新全段审阅；第 2 节保留 S01 的时间片段转述 |
| <a id="ref-s05"></a>S05 | 历史完整执行包及台账 | 当前缺失；不是已验证引用，不猜其内容 |
| <a id="ref-s06"></a>S06 | 用户真实代码与设备环境 | 本次未访问；S01 的历史目录观察需重新检查 |

### 20.2 本次官方文档核验

核验日期：2026-09-11。以下为非中文官方资料。文中 O 编号只支持相应产品文档事实；具体架构、优先级、测试目标和工程取舍属于本项目设计。权限、额度、版本和供应商接口在实施当天继续核验。

| 编号 | 主题 | 状态与使用边界 | 官方来源 |
| --- | --- | --- | --- |
| <a id="ref-o01"></a>O01 | Codex 项目实践 | 本次读取。用于支持明确任务、真实环境、测试和审查的工作方式；不提供本项目完成比例。 | [Codex 项目实践](https://developers.openai.com/codex/learn/best-practices) |
| <a id="ref-o02"></a>O02 | Codex AGENTS.md | 本次读取。支持按作用域组织规则；总纲保持独立，项目指令只索引所需章节。 | [Codex AGENTS.md](https://developers.openai.com/codex/guides/agents-md) |
| <a id="ref-o03"></a>O03 | Playwright Trace Viewer | 本次读取。可查看执行动作、快照和网络；原始记录可能包含请求细节，需独立隐私审查。 | [Playwright Trace Viewer](https://playwright.dev/docs/trace-viewer) |
| <a id="ref-o04"></a>O04 | Playwright Tracing API | 本次读取。浏览器 tracing 与测试断言记录范围有区别。 | [Playwright Tracing API](https://playwright.dev/docs/api/class-tracing) |
| <a id="ref-o05"></a>O05 | Playwright BrowserType | 本次读取。持久会话目录须独立管理，不让多个实例共用同一目录。 | [Playwright BrowserType](https://playwright.dev/docs/api/class-browsertype) |
| <a id="ref-o06"></a>O06 | Supabase Queues | 本次读取。消息交付语义限定可见性窗口，不能外推到第三方副作用。 | [Supabase Queues](https://supabase.com/docs/guides/queues) |
| <a id="ref-o07"></a>O07 | Supabase pgmq 与队列 API | 本次读取/检索官方说明。确认可见性、读取与删除语义；具体事务集成在本项目实测。 | [Supabase pgmq 与队列 API](https://supabase.com/docs/guides/queues/pgmq) |
| <a id="ref-o08"></a>O08 | Playwright Mock APIs | 本次读取。支持网络 Mock/HAR 相关测试；不能据此承诺完整线上环境重演。 | [Playwright Mock APIs](https://playwright.dev/docs/mock) |
| <a id="ref-o09"></a>O09 | Supabase Row Level Security | 本次读取。高权服务角色可绕过 RLS，业务授权仍需校验。 | [Supabase Row Level Security](https://supabase.com/docs/guides/database/postgres/row-level-security) |
| <a id="ref-o10"></a>O10 | Vercel 函数时限 | 本次读取。存在按部署方案配置的执行时限；不在规划中锁定固定秒数。 | [Vercel 函数时限](https://vercel.com/docs/functions/configuring-functions/duration) |
| <a id="ref-o11"></a>O11 | TikTok Content Sharing Guidelines | 本次读取。Direct Post 的目标用途限制不接受仅服务自己或团队账号的内部上传工具；不能默认审核获准。 | [TikTok Content Sharing Guidelines](https://developers.tiktok.com/docs/en/content-sharing-guidelines) |
| <a id="ref-o12"></a>O12 | WhatsApp Business Messaging Policy | 本次读取。按客户会话窗口、模板、联系依据、退出和人工升级要求设计；实际账号资格另核验。 | [WhatsApp Business Messaging Policy](https://business.whatsapp.com/policy) |
| <a id="ref-o13"></a>O13 | Google Places Policies | 本次读取。字段保存、缓存和署名有边界；place ID 的例外不推广到所有详情。 | [Google Places Policies](https://developers.google.com/maps/documentation/places/web-service/policies) |
| <a id="ref-o14"></a>O14 | scrcpy 官方仓库 | 本次读取。提供 Android 显示和控制能力；业务调度和结果验收由项目另建。 | [scrcpy 官方仓库](https://github.com/Genymobile/scrcpy) |
| <a id="ref-o15"></a>O15 | Appium 非 macOS 宿主 | 本次读取。Windows/Linux 为有限支持，实际真机、系统、WDA 部署和版本须满足文档条件。 | [Appium 非 macOS 宿主](https://appium.github.io/appium-xcuitest-driver/latest/guides/non-macos-hosts/) |
| <a id="ref-o16"></a>O16 | Meta 官方 Instagram Postman 文档 | 本次读取官方入口。具体发布/消息对象的权限与分步调用，在适配任务中按所选接入路径继续核验。 | [Meta 官方 Instagram Postman 文档](https://www.postman.com/meta/instagram/documentation/6yqw8pt/instagram-api) |
| <a id="ref-o17"></a>O17 | TikTok Business Messaging 资料入口 | 沿用 S01 的官方入口；本次抓取失败，不据此确认当前产品细节、资格、接口或账号可用性。 | [TikTok Business Messaging 资料入口](https://ads.tiktok.com/gateway/docs/index?doc_id=1735712062490625) |
...
本版已完成文档修订、章节组织、任务与验收条目编排。第 28 节的任务、拟定脚本、接口、Schema 和测试均属于待实施规格。本次不输出应用代码或声明产品测试已通过。

保留 S01 全部 12 个模块、20 个工作区、31 个主包和 36 项原验收；工程增强追加新编号。S01 的生产目标、数据保留、权限和订单原则继续有效，已在相应章节显式修订有歧义处。

核心差异如下：

| 原版位置 | 本版处理 | 理由 |
| --- | --- | --- |
| 第 1 节检查范围 | 按本次真实读取情况重写，旧 Windows 检查标为历史 | 防止复制旧环境事实 |
| 第 5 节首链 | 分 E1 技术链与 E2 成交链 | 先消除执行器风险，同时保留业务目标 |
| 第 6 节页面 | 完整保留，G1 限五个最小工作区 | 防止界面进度领先真实功能 |
| 第 8、9 节队列 | 原子投递原则保留，按事务边界选择直接入队或 Outbox | 防止双写漏洞和无依据冗余 |
| 第 9 节租约/接管 | 增加在途动作和旧进程隔离边界 | 数据库 token 无法撤回外部请求 |
| 第 12 节能力 | 证据状态与运行模式分开，增加受控试验许可 | 消除首次验证的状态循环 |
| 第 15 节顺序 | 使用 G0 至 G5，旧主包编号保持不变 | 浏览器和基础诊断前置 |
| 第 17 节验收 | 原 ACC 保留，新增诊断、回归、恢复和安全验收 | 不把新规格伪装成原清单 |
| 第 19 节 Codex | 补充原子任务、脚本基线、并行边界与验收证据 | 降低跨任务与上下文失真 |
| 第 22 至 30 节 | 新增详细执行规则、字段、模板及任务拆解 | 将总纲转换为实际开发入口 |

来源缺口仍然存在：S04 未重新完整观看，S05 未取得，真实仓库和外部账号未连接。以上缺口是开工台账的一部分，不由文档篇幅消除。
...
本节为 v2 新增设计，承接 S01 第 7、9、13 节。它规定最先验证的工程范围，不能被后期完整页面替代。
...
| 内容 | 一条批准文本或一个已校验附件版本 | AIGC、批量视频裂变 |
...
| 安全 | 品牌范围、密钥隔离、提交前检查、诊断脱敏 | 不允许为了快而省掉这些边界 |
...
**G1-B：真实账号只读。** 通过获准访问路径启动环境，读取可验证的账号身份和一个允许对象，返回来源与结果。可验证会话、网络、宿主和定位适配；不证明发送或发布可用。
...
至少通过：重复请求、错误账号、授权失效、提交前停止、提交后断网、旧命令重放、Agent 重启和日志脱敏。仅能读取就记为“真实只读通过”；真实写入没有证据，整个写入范围仍为待验证。

<a id="section-23"></a>

## 23. RPA Debug Bundle：失败诊断包

本节为 v2 新增工程能力 `PLAN-X-01`，对应 MOD-07/MOD-12。目标是让维护者和 Codex 能在不暴露客户资料的情况下复现定位与状态问题。
...
| ENV-L3 底层可选研发 | 维护自有浏览器分支或更底层能力的独立立项 | 单独需求、版本维护预算、安全更新和兼容测试 | 不纳入默认首期，保留待决需求 |

独立目录不能证明 ENV-L2 已完成。第三方能力接入可实现产品功能，但须显示供应商依赖、授权与续费；它也不等同于自研底层所有权。

ENV-L3 是否纳入完整交付由范围决定。用户未同意删减时，缺少底层能力保持未完成/待决，不用“可选”字样偷偷移出原始需求。
...
本节为 v2 新增执行分解，共 134 个细任务，映射 31 个原主包及 4 个新增工程包。任务状态初始全部为 TODO，任何任务都尚未因这份文档而实施。

### 28.1 粒度与依赖规则

每一行提供任务边界、交付与最低测试预期。开始前按第 30 节模板补齐实际仓库、文件白名单、输入、脚本和回滚；缺少这些不允许把整张表作为一条“大任务”执行。

依赖指“当前所请求范围的前置结果”，不是父主包全部功能已经完成。G1 切片复用同一公共类型和状态机；多个任务依赖公共基础时不各自重新造一套。
...
单个适配器任务默认只能改它自己的目录、必要的注册表及对应测试。公共 Schema、权限和状态机变化必须在任务卡中显式扩大授权范围，并添加兼容测试。生产数据、真实凭据、未授权文件与其他用户未提交工作不在默认修改范围。
...
| --- | --- | --- | --- |
| <a id="task-001"></a>TASK-001 材料与版本清单<br>PLAN-F0-01 | 无 | 定位 S01 至 S05 的实际文件和哈希；缺失项保留；不猜旧需求编号 | 从当前文件生成清单并可回查；文档检查 |
...
| <a id="task-009"></a>TASK-009 ENV-L1/L2/L3 范围卡<br>PLAN-X-03 | TASK-003、TASK-008 | 记录厂商声明、已知隔离与可选底层研发边界 | 普通 profile 隔离不冒充 ENV-L2；ACC-46 |
...
| <a id="task-014"></a>TASK-014 G0 关口与下一切片<br>PLAN-F0-05 | TASK-005、TASK-011、TASK-012、TASK-013、TASK-009、TASK-010 | 输出首个开发任务、真实阻塞和授权请求 | 资料缺口只阻塞对应冻结；没有采购或外发；文档检查 |

### 28.4 G1：单账号可靠执行闭环，29 个任务
...
| <a id="task-020"></a>TASK-020 单账号连接与身份<br>PLAN-F1-02 | TASK-017、TASK-018 | 创建连接器凭据引用，验证预期外部账号，OAuth 路径验证 state | 错账号、撤权、回调重放和并发刷新；ACC-02、ACC-54、ACC-56 |
| <a id="task-021"></a>TASK-021 单条内容版本<br>PLAN-F1-03 | TASK-017 | 实现一条文本或附件元数据与不可变摘要 | 内容变化生成新版本，不覆盖旧批准；ACC-03 |
...
| <a id="task-031"></a>TASK-031 独立环境启动停止<br>PLAN-F3-01 | TASK-029、TASK-008 | 接入选定 profile 路径，绑定账号与 runtime 状态 | 双环境存储隔离，不能复用日常目录；ACC-46、ACC-47 |
...
| <a id="task-035"></a>TASK-035 单一只读模板<br>PLAN-F3-02 | TASK-032、TASK-018、TASK-034 | 实现定位、账号核验、对象读取和来源结果 | 错误身份/多候选元素安全停止；ACC-30 |
...
| <a id="task-038"></a>TASK-038 E1 本地闭环故障验收<br>PLAN-F3-07 | TASK-035、TASK-036、TASK-037、TASK-025 | 合成输入经数据库、队列、Agent 到结果页完整运行 | 重复、杀 Worker、断网、停止与恢复全部留证；ACC-07、ACC-08、ACC-09、ACC-13 |
| <a id="task-039"></a>TASK-039 真实只读连通试验<br>PLAN-F3-02 | TASK-038、TASK-020、TASK-019、TASK-007 | 获准账号读取预期对象并保存脱敏证据 | 明确 L3 只读范围；不能提升发布/消息能力；ACC-43 |
| <a id="task-040"></a>TASK-040 单个获准写动作适配<br>PLAN-F1-05 | TASK-039、TASK-021、TASK-022、TASK-037 | 按已核实 API 或 UI 路径实现准备、提交和远端核实 | 上传/受理/最终发布分开；框架级写入重试关闭；ACC-11、ACC-14 |
...
| <a id="task-042"></a>TASK-042 一次受控真实写入<br>PLAN-F1-05 | TASK-040、TASK-041、TASK-038 | 依据明确许可执行一个目标，取得远端证据或标未知 | 没有真实证据不宣布写入通过，原生平台结果可核对；ACC-11、ACC-41 |
| <a id="task-043"></a>TASK-043 G1 范围放行<br>PLAN-F3-07 | TASK-042、TASK-032、TASK-036 | 核对 E1、关键故障、身份隔离、控制和只读/写入差异 | 未获准写入则关口受阻范围单列；ACC-43、ACC-44、ACC-63 |

### 28.5 G2：采集、模板、小规模矩阵与维护，26 个任务

| 细任务 / 父主包 | 直接依赖 | 交付边界 | 必须验证 |
| --- | --- | --- | --- |
| <a id="task-044"></a>TASK-044 D1 脱敏现场与导出<br>PLAN-X-01 | TASK-033、TASK-043 | 按需掩码图像、安全 DOM、网络元数据及导出审查 | 跨渠道秘密扫描和未授权下载拒绝；ACC-37、ACC-57 |
...
| <a id="task-048"></a>TASK-048 首个真实数据源读取<br>PLAN-F2-01 | TASK-047、TASK-007 | 实现一项获准对象的分页和来源回执 | 来源限制与样本范围明确，不承诺全量；ACC-50 |
...
| <a id="task-051"></a>TASK-051 Excel/CSV 导入导出<br>PLAN-F2-01 | TASK-049 | 映射、预览、错误行、去重、授权字段和编码处理 | 前导零、长 ID、公式注入往返验收；ACC-16 |
...
| <a id="task-054"></a>TASK-054 联系资格与退出基础<br>PLAN-F2-02 | TASK-018、TASK-053 | 来源、用途、窗口、退出、账号范围的服务端规则 | 高意向不绕过资格，队列退出即时影响后续提交；ACC-18、ACC-19 |
...
| <a id="task-061"></a>TASK-061 配额、预算与公平排队<br>PLAN-F1-04 | TASK-060、TASK-041 | 按品牌/账号/平台公平调度，费用预占结算分开 | 竞争下不超内部预算，未知保留待核算；ACC-58、ACC-66 |
| <a id="task-062"></a>TASK-062 多执行槽与资源约束<br>PLAN-F3-01 | TASK-032、TASK-061 | 从一个槽到小规模可配置槽位，资源不足回压 | 同 profile 不并发，积压可见且不失任务；ACC-31 |
...
| <a id="task-064"></a>TASK-064 ENV-L2 供应商适配<br>PLAN-X-03 | TASK-009、TASK-031 | 有正式接口才接入声明范围、回读和升级兼容 | 超时、重复启动、授权到期和停止确认；ACC-47 |
| <a id="task-065"></a>TASK-065 外部 URL 与文件输入约束<br>PLAN-F3-01 | TASK-064、TASK-047 | 主机/协议白名单、目标归属、下载限额与路径安全 | 拒绝内网、元数据、重定向逃逸和任意本地路径；ACC-48 |
...
| <a id="task-069"></a>TASK-069 G2 竞品核心范围验收<br>PLAN-F3-07 | TASK-068、TASK-058、TASK-062、TASK-066、TASK-063 | 限定规模验证数据、模板、计划、环境与维护材料 | 局部阻塞保留；未知与人工比例按版本显示；ACC-36、ACC-64 |

### 28.6 G3：内容、接待、客户和成交闭环，34 个任务

| 细任务 / 父主包 | 直接依赖 | 交付边界 | 必须验证 |
| --- | --- | --- | --- |
| <a id="task-070"></a>TASK-070 媒体输入校验<br>PLAN-F2-03 | TASK-021、TASK-069 | 对象权限、扩展名与真实格式、大小/时长、下载策略 | 异常容器和恶意文件不能拖垮宿主；ACC-48、ACC-60 |
...
| <a id="task-077"></a>TASK-077 按平台逐动作发布扩展<br>PLAN-F2-04 | TASK-076、TASK-067 | 渠道规格和实际能力分开管理，每种动作继续拆子任务 | 上传、处理、发布与不可得回执分别记录；ACC-11、ACC-59 |
| <a id="task-078"></a>TASK-078 一条真实收件入口<br>PLAN-F1-06 | TASK-017、TASK-025、TASK-007 | 原始事件验签、限额、可靠持久化再确认 | 重复/乱序/伪造事件和存储失败正确处理；ACC-10、ACC-72 |
...
| <a id="task-080"></a>TASK-080 人工回复动作<br>PLAN-F1-06 | TASK-079、TASK-054 | 正文版本、窗口、模板/用途和退出检查接入共用执行器 | 不允许绕过既有任务安全内核；ACC-19、ACC-25 |
...
| <a id="task-087"></a>TASK-087 客户档案与跟进<br>PLAN-F2-06 | TASK-079、TASK-055 | 客户、渠道身份、负责人、阶段和时间线 | 咨询人数和消息条数分开；来源未知保留；ACC-20、ACC-55 |
...
| <a id="task-091"></a>TASK-091 落地页与号码池分流<br>PLAN-F1-07 | TASK-090 | 来源参数、号码状态、分流记录和页面发布 | 点击号码链接仅计触点；无效号码暂停分流；ACC-28 |
...
| <a id="task-100"></a>TASK-100 费用与经营报表<br>PLAN-F2-07 | TASK-061、TASK-095、TASK-099 | 各币种收款/退款/费用，未知成本和预算预占分开 | 未发生费用不猜值，零分母显示无样本；ACC-66 |
| <a id="task-101"></a>TASK-101 选定消息渠道真实测试<br>PLAN-F1-06 | TASK-086、TASK-091、TASK-007 | 若选择 WhatsApp，则核验窗口、模板、退出和人工升级 | App 个人使用与商业 API 权限分开；ACC-25；ACC-62 |
| <a id="task-102"></a>TASK-102 E2 内容到交付闭环<br>PLAN-F1-08 | TASK-077、TASK-092、TASK-096、TASK-099、TASK-100、TASK-101 | 使用获准测试咨询和支付测试模式完成全链 | 技术测试、受控真实测试、实际营业结果分开；ACC-36 |
| <a id="task-103"></a>TASK-103 G3 业务范围验收<br>PLAN-F1-08 | TASK-102、TASK-097、TASK-086 | 核实完整 UI、异常、数据关系与对账 | 无真实能力证据的渠道不标完成；ACC-36、ACC-63、ACC-64 |
...
G0 先只读，不先创建所有代码目录。G1 先建立最小公共合同和权限，再接通一个实际 Agent 与一个模板；每个局部任务有测试，最后用 E1 把账号、任务、队列、执行器、真实结果和页面一次串起来。
...
| G1 能力 | capability_checks、pilot_permits、approval_decisions | 能力版本、限范围试验和批准快照 |
| G1 传输与现场 | agents、environments、inbound_events、diagnostic_bundles；按 ADR 选 outbox | 消息交付与业务状态分开，诊断默认最小化 |
| G2 数据与调度 | collection_queries、observations、checkpoints、schedules、contact_permissions、suppressions | 分页一致、源端覆盖说明、退出和范围快照 |
...
素材上传校验访问范围、大小、真实格式与处理时限。媒体解码在受限进程运行，不将用户参数拼成 shell 命令。下载阻断未授权内网、云元数据、路径穿越和重定向绕过。
...
134 个任务是边界明确的执行目录，不代表 134 天，也不意味着每次 Codex 对话必须严格完成一行。过大的一行可继续加 `TASK-xxx-a` 等子编号，保留父关系；共享很小的纯数据变更可经批准合并。
...
本节是可直接给 Codex 使用的入口。开始实际执行前，需要它按当前工作区重新定位本文中的路径和任务范围。

### 30.1 第一次复制给 Codex 的指令

```text
请读取 KFF_项目规划_v2.0_Codex执行版.md 的第 0、1、15、19、22、28、30 节。

本轮只执行 TASK-001 和 TASK-002 的只读检查。
本轮不创建应用、安装依赖、修改数据库、改动生产、发送消息、发布内容或采购。

先读取当前仓库和目录作用域中的 AGENTS.md / AGENTS.override.md，
确认当前工作目录、分支、未提交改动、已有锁文件、运行脚本与部署记录。
只访问当前工作区和已明确授权的相关目录，不扫描无关个人文件。

读取本工作区能实际找到的原始资料，列出文件路径、版本、哈希和缺失项。
若历史执行 ZIP 不存在，不猜110项需求或90项验收的编号和内容。
不要根据历史聊天中的空仓库结论代替本次实际检查。

输出：
1. 当前仓库与资料的事实清单。
2. 存量模块的待核查列表及可访问路径。
3. 本次没有执行的检查与原因。
4. 推荐的下一项最小任务，包含真实文件白名单、测试方式和回滚边界。
5. 下一任务需要补充的账号、接口或设备信息，只问确实无法从现有资料查明的内容。
...
```text
本轮执行 TASK-xxx，不执行其他功能扩展。
先核对本任务的父包、关口、依赖、输入输出、适用验收和真实代码路径。
读取已有项目规则，确认依赖已在本轮要求的范围内通过。

请先给出本轮文件白名单与要执行的实际测试命令。
按已批准任务实现一个完整小切片，复用现有主数据与执行内核。
发现需求冲突、缺少第三方接口、范围扩大或可能产生未授权外部动作时停止该部分并报告。

不能用 Mock、占位页面、固定成功返回或手工代操作冒充真实功能。
不要修改无关架构、覆盖项目规则或顺手更新全项目依赖。
测试应覆盖正向、负向和相关故障路径；必须报告实际命令、退出码、样本数与结果。

交付：修改摘要、文件清单、迁移与兼容说明、测试证据、未完成项、回滚方法，
并更新该任务及能力台账的准确状态。完成后停止，不自动转入下一任务。
```
...
离线通过后给出受控真实复测所需许可与目标，不自行晋升生产能力。
...
这是格式模板，空字段不能作为执行前已满足。尤其是文件白名单、真实写入许可、验收和回滚，需要在具体任务批准前落实。最小字段不足时可以只完成只读发现，不进入写入实现。
...
不同适配器、不同页面或独立测试样例可以在公共合同稳定后并行，但不得无协调地同时修改数据库迁移、共享状态机、权限模型、同一工作树或同一真实环境。

为并行任务分配独立分支或工作树、文件责任和测试账号；合并后运行合同与公共回归。Agent 数量增加不能替代依赖梳理，也不保证等比例提速。[O01](#ref-o01)

一次工作中发现的新需求进入后续待办，不在当前 PR 中顺手建设另一个系统。任务卡中的“下一项”是建议，未获准时不自行执行。

### 30.7 用户的最小验收动作

用户每轮只需确认可观察结果：用了哪个账号、任务是否真的执行、失败有没有准确显示、是否出现重复动作、相关业务数据能否对得上。测试许可明确目标和次数，不直接提供生产全权给所有开发任务。
...
**正式开工入口：先执行 TASK-001 与 TASK-002 的只读检查，再按依赖推进最小真实执行链。**

本次使用模型：GPT-6 Astra Pro。
