...
> **历史文档（保留原日期与原结论，不再作为当前依据）。**
> 当前状态以 [`docs/current-status.md`](current-status.md) 为唯一入口，缺陷与修复状态以
> [`docs/defect-ledger.json`](defect-ledger.json) 为准。
>
> 引用本文请注意三点：
> 1. 本文的 **64 项 / 已修复 8 / 剩余 56** 是当时的统计，**不得与本轮 25 项相加**，也**不得据此计算完成率**：本文剩余段的 56 项尚未逐条映射（台账记为 `TO_VERIFY`）。
> 2. 本文"已修复"中至少两条已有当前源码反例：第 3 条（任务创建表单初始化与过滤条件不一致）→ 台账 `KFF-B07` **重新打开**；第 6 条（采集结果导出 100% 失败）→ 台账 `KFF-B08` **部分重开**（固定快照出口仍用旧字段规则）。
> 3. 基线与本文不同：本文审计于 `2da983e`，当前台账基线为 `891f399a`。
