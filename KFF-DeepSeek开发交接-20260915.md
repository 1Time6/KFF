...
- 启动方式说明：PowerShell 的 `Start-Process` 在含 `NO_PROXY`/`no_proxy` 两个大小写变量的环境里会报 “Item has already been added”，改用 `.kff/relaunch-local-runtime.ts`（等价入口、detached）启动宿主，其余步骤仍走原 `local-runtime.ts configure/status/stop`。`local-runtime.ps1 -Action Stop/Status` 正常可用，只有 Start 需要绕过。
