...
while (!stopped) {
  try { await recoverExpired(); await dispatchOne(); await processCollectionPage(); if (Date.now() - lastRetentionCheck > 60000) { await purgeExpiredCollectionData(); lastRetentionCheck = Date.now(); } }
