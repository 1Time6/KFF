...
export function InboxWorkbench({data}:{data:Workspace}) {
  ...
  return <div className="business-workbench">
    ...
    {selected?<ConversationPanel key={selected} id={selected}/>:<div className="business-box business-empty">选择会话，查看客户消息与服务窗口。</div>}
    </div>
  </div>;
}
...
function ConversationPanel({id}:{id:string}){
  ...
  return <section className="business-box conversation-panel" aria-label="会话详情">
    {detail&&<><div className="business-box-head"><div><h2>{detail.conversation.display_name??'匿名访客'}</h2><p>{detail.conversation.channel_name} · {detail.conversation.is_synthetic?'测试咨询':'站内咨询'}</p></div><Link href={'/customers?customer='+detail.conversation.customer_id}>客户档案 →</Link></div>
    ...
    <ol className="message-stream" aria-label="已保存的客户消息">{messages.map(message=><li key={message.id}><div className="message-bubble"><p>{message.body}</p></div><small>#{message.sequence} · {businessTime(message.received_at)} · 已收件</small></li>)}</ol>
    <div className="conversation-footer"><button className="button subtle" onClick={()=>void refresh()}>{detail?.has_more?'加载后续消息':'刷新消息'}</button><p>人工回复功能正在接入。当前可以收取咨询、维护客户和记录跟进。</p></div>
  </section>;
}
