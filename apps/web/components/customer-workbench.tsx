...
import {LeadCustomer} from './lead-customer';
...
function CustomerPanel({id,members,scope,onSaved}:{id:string;members:CustomerData['members'];scope:Workspace['scope'];onSaved:()=>Promise<void>}){
  ...
  return <section className="business-box customer-panel" aria-label="客户详情">{error&&<div className="alert warning" role="alert">{error}<button onClick={()=>void refresh()}>刷新后重新核对</button></div>}{notice&&<p role="status" className="business-success">{notice}</p>}
    {detail&&<><div className="business-box-head"><div><h2>{detail.customer.display_name??'匿名访客'}</h2><p>首次咨询 {businessTime(detail.customer.created_at)}</p></div><span className="badge neutral">{stageNames[detail.customer.stage]}</span></div>
      <div className="customer-origin"><p><strong>建档来源</strong> {detail.customer.acquisition_source?'Facebook 客户互动':'KFF 站内主动咨询'}</p><p><strong>支付核实</strong> {detail.payment_summary.real||detail.payment_summary.test?`真实付款 ${detail.payment_summary.real} 笔 · 测试记录 ${detail.payment_summary.test} 笔`:'尚无记录'}</p><small>成交、交付阶段为人工标记；款项核实和交付证据分别记录。</small></div>
      <LeadCustomer customer={detail.customer} writable={scope.role!=='viewer'} onSaved={async()=>{await refresh();await onSaved();}}/>
      ...
      <div className="customer-timeline"><h3>跟进时间线</h3><ol>{detail.events.map(event=><li key={event.id}><strong>{event.event_type==='INQUIRY'?'收到主动咨询':event.event_type==='NOTE'?'跟进备注':event.event_type==='AI_DECISION'?'自动接待判断':event.event_type==='LEAD_UPDATED'?'更新客户意向':'更新客户档案'}</strong><time>{businessTime(event.created_at)}</time>{event.event_type==='NOTE'&&<p>{event.details.text}</p>}{event.event_type==='AI_DECISION'&&<p>{event.details.decision?.reason}</p>}{event.event_type==='LEAD_UPDATED'&&<p>{event.details.reason}</p>}{event.event_type==='PROFILE_UPDATED'&&<p>{event.details.reason} · {stageNames[event.details.result.stage]}</p>}</li>)}</ol><p className="muted">显示最近 200 条记录。</p></div>
    </>}
  </section>;
}
