...
export function ActionAdjudication({ run, task, records, role, onSaved }: { run: Run; task: Task; records: AdjudicationRecord[]; role: Scope['role']; onSaved(): Promise<void> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const value = { snapshot_hash: task.snapshot_hash, expected_version: version, expected_state: expectedState, decision, evidence: { source: String(form.get('source')), external_account_id: String(form.get('actual_account_id')), content_hash: task.snapshot.content_hash, ...((task.snapshot.message||task.snapshot.outreach)?{recipient_id:String(form.get('recipient_id'))}:{}),...(task.snapshot.message?.browser?{thread_id:String(form.get('thread_id'))}:{}), ...(task.snapshot.outreach?.browser?{parent_id:String(form.get('parent_id')),source_url:String(form.get('source_url'))}:{}), remote_id: String(form.get('remote_id')).trim() || null, observed_at: new Date(String(form.get('observed_at'))).toISOString(), reference: String(form.get('reference')), failure_basis: decision === 'CONFIRMED_FAILURE' ? String(form.get('failure_basis')) : null, matched_original_submission: decision !== 'INCONCLUSIVE' && form.get('matched') === 'on' }, reason: String(form.get('reason')), confirmation: 'I_REVIEWED_THIS_ORIGINAL_ACTION' };
  ...
  return <section className="adjudication-panel" aria-label="人工复核与裁定">
    ...
    {eligible && role === 'admin' && <details className="adjudication-form"><summary>登记新的人工裁定</summary>
      ...
      <form key={version} onSubmit={event => void submit(event)}><fieldset disabled={busy || stale}>
        ...
        {task.snapshot.outreach?.browser&&<><Field label="原评论 ID"><input name="parent_id" defaultValue={task.snapshot.outreach.browser.comment_id} required/></Field><Field label="新回复完整链接"><input name="source_url" type="url" required={decision==='CONFIRMED_SUCCESS'} placeholder={task.snapshot.outreach.browser.comment_url+'&reply_comment_id=新回复ID'}/></Field></>}{task.snapshot.body && <div className="content-preview"><label>需要核对的原内容</label><p>{task.snapshot.body}</p></div>}
        {(task.snapshot.message||task.snapshot.outreach)&&<Field label="证据中实际收件人 ID"><input name="recipient_id" required pattern="[0-9]{1,128}" inputMode="numeric" /></Field>}
        {task.snapshot.message?.browser&&<Field label="证据中实际会话 ID"><input name="thread_id" required maxLength={160}/></Field>}
