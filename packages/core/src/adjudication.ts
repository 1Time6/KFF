...
export async function adjudicateAction(scope: Scope, runId: string, input: z.infer<typeof adjudicationInput>) {
  ...
  return scoped(scope, async client => {
    ...
    if(snapshot.message)requireCondition(value.evidence.recipient_id===snapshot.message.contact.remote_id,'ACCOUNT_MISMATCH','证据中的客户与原私信收件人不符',409);
    ...
    if (value.decision === 'CONFIRMED_SUCCESS') {
      ...
      if (isWrite(snapshot)) {
        ...
        requireCondition((snapshot.is_synthetic ? /^synthetic_[a-f0-9-]{36}$/ : snapshot.message?/^[A-Za-z0-9_:.=$+/-]{1,200}$/:/^[0-9]+_[0-9]+$/).test(remoteId), 'INVALID_INPUT', '远端对象标识与本动作类型不符');
