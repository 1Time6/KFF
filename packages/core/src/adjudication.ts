  ...
  return scoped(scope, async client => {
    ...
    if (value.decision === 'CONFIRMED_SUCCESS') {
      ...
      if (isWrite(snapshot)) {
        ...
        requireCondition((snapshot.is_synthetic ? /^synthetic_[a-f0-9-]{36}$/ : /^[0-9]+_[0-9]+$/).test(remoteId), 'INVALID_INPUT', '远端对象标识与本动作类型不符');
    ...
    const receipt = value.decision === 'INCONCLUSIVE' ? action.receipt : { evidence_kind: 'human_review', adjudication_id: record.id, remote_id: value.evidence.remote_id, actual_account_id: snapshot.external_account_id, content_hash: snapshot.content_hash, observed_at: value.evidence.observed_at };
