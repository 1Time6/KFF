...
export async function exportCollection(scope:Scope,id:string,input:z.infer<typeof collectionExportInput>) {
  ...
  return scoped(scope,async client => {
    ...
    const rows=(await client.query<CollectionResult>('SELECT r.id,r.observation_id,r.result_order::text,o.source_object_id,o.observed_at,o.source_url,o.fields,o.evidence_hash,o.allowed_purposes,o.expires_at,o.object_version FROM kff.collection_results r JOIN kff.collection_observations o ON o.id=r.observation_id WHERE r.run_id=$1 AND ($2::uuid[] IS NULL OR r.id=ANY($2)) AND o.expires_at>clock_timestamp() ORDER BY r.result_order LIMIT 1001',[run.id,value.result_ids??null])).rows;
    ...
    const headers=['source_object_id',...value.fields.flatMap(field=>[field,field+'__kind']),'source_type','source_url','observation_id','observed_at','expires_at','evidence_hash','allowed_purposes','query_id','returned_count','unique_count','reported_total','coverage','_kff_text_encoding'];
    ...
    const table=rows.map(row=>{
      ...
      data.push(text(snapshot.source_type),text(row.source_url),text(row.observation_id),text(new Date(row.observed_at).toISOString()),text(new Date(row.expires_at).toISOString()),text(row.evidence_hash),text(row.allowed_purposes.join(',')),text(id),run.returned_count,run.unique_count,run.reported_total??'',snapshot.source_type==='MANUAL_IMPORT'?'UPLOADED_ROWS_ONLY':'SYNTHETIC_SAMPLE',isCSV?'apostrophe-v1':'typed-xlsx-v1'); return data;
    });
    let bytes:Buffer;
    ...
    await audit(client,scope,'collection.exported',id,{format:value.format,fields:value.fields,exported_objects:rows.length,observation_ids:rows.map(row=>row.observation_id),file_hash:hash,expires_at:query.expires_at,coverage:snapshot.source_type==='MANUAL_IMPORT'?'UPLOADED_ROWS_ONLY':'SYNTHETIC_SAMPLE'});
