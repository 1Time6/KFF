...
export async function ensureBundledTemplates(client: PoolClient, scope: Scope) {
  for (const key of templateCapabilityKey.options) {
    const manifest = fixedPageManifest(key); const name = (key.startsWith('kff.fixture.') ? '本地合成 · ' : 'Facebook · ') + (key === 'facebook.comment.reply.browser'?'公开评论回复':key.includes('.inbox.')?'收件读取':key.includes('.discovery.')?'分页采集':key.includes('.reply.')?'私信接待':key.includes('.publish.') ? '文本发布' : '身份读取');
