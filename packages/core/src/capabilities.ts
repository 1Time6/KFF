...
export async function attachLocalEvidence(scope: Scope, capabilityId: string) {
  requireAdmin(scope); return scoped(scope, async client => {
    ...
    requireCondition(capability && !capability.is_synthetic && capability.adapter_version === 'facebook-graph-v1', 'NOT_FOUND', '未找到此 Facebook 能力', 404);
    ...
    const artifact = (await client.query('SELECT * FROM kff.adapter_artifacts WHERE id=$1 AND adapter_version=$2', [hash, capability.adapter_version])).rows[0];
