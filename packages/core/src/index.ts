...
export function buildDiagnostic(report: { step: string; scene?: Record<string, number> }, errorCode: string | undefined, scope: { organization_id: string; brand_id: string }, actionId: string) {
  ...
  return {
    schema_version: 'kff.diagnostic.v1', organization_id: scope.organization_id, brand_id: scope.brand_id, action_id: actionId,
    level: scene ? 'D1' : 'D0', step: report.step, error_code: errorCode ?? null,
