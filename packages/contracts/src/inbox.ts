...
export interface Customer {id:string;display_name:string|null;owner_user_id:string|null;stage:typeof customerStages[number];version:number;created_at:string;updated_at:string;first_inquiry_event_id:string;lead_status:import('./lead').LeadStatus;tags:string[];intent_level:string;intent_category:string|null;intent_reason:string|null;valid_inquiry:boolean;first_interaction_at:string|null;last_interaction_at:string|null;acquisition_source:Record<string,unknown>|null}
export interface InboxConversation {id:string;customer_id:string;account_id:string;channel_id:string|null;channel_kind:string;handling_mode:'AI'|'HUMAN'|'PAUSED';control_version:number;last_inbound_sequence:number;lead_status:Customer['lead_status'];intent_level:string;tags:string[];valid_inquiry:boolean;identity_id:string;last_sequence:number;last_message_at:string;reply_window_expires_at:string|null;display_name:string|null;stage:Customer['stage'];owner_user_id:string|null;channel_name:string;is_synthetic:boolean;opted_out:boolean;contact_target_id:string}
export interface InboxMessage {id:string;conversation_id:string;sequence:number;direction:'INBOUND'|'EXTERNAL_OUTBOUND'|'OUTBOUND';body:string;received_at:string;client_sent_at:string|null;inbound_event_id:string|null;message_kind?:string;source?:Record<string,unknown>|null;actor_kind?:string;action_id?:string|null}
