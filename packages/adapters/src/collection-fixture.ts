...
export function normalizeCollectionPage(value: unknown, request: CollectionRead) {
  ...
  for (const row of page.rows) {
    ...
    requireCondition(url.origin === 'http://127.0.0.1:4311' && url.pathname === '/collection-object/' + row.source_object_id && !url.search && !url.hash, 'COLLECTION_SOURCE_MISMATCH', '合成来源链接不匹配', 409);
