...
依据《项目规划.md》v2.1 实施。按用户明确决定，KFF 独立拥有客户、订单、聊天与数据，见 [ADR-017](docs/decisions/017-independent-business-data.md)。目前已有本地执行、数据导入与查询、计划规则、自有咨询收件、客户及商品/订单快照基础；完整项目、人工回复、支付/退款/交付、真实 Facebook 和设备验收仍在后续范围。
...
16. 商品与订单页可保存商品价格、交付范围和条款的新版本，管理员启用后为已有客户生成报价预览，再核对币种、总额与条款确认订单。商品改价不影响历史订单；过期预览、旧价格或错误客户会话会被拒绝。当前订单可取消，支付状态始终为待核实；实际支付、退款、交付继续留在后续任务。见 [独立订单 API](docs/api/orders.md)。
